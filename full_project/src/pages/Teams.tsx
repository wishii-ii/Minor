...
import { FaCrown, FaUser, FaLock, FaUsers } from 'react-icons/fa';
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
        <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-white">Create New Team</h2>
        <form onSubmit={handleSubmit}>
          
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Team Avatar</label>
            <div className="flex flex-wrap gap-2 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
              {emojis.map(e => (
                <button
                  ...
                  type="button"
                  onClick={() => setAvatar(e)}
                  className={`text-2xl p-2 rounded-full transition-all duration-150 ${
                    avatar === e 
                      ? 'ring-4 ring-indigo-500 bg-indigo-100 dark:bg-indigo-900/50' 
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 bg-white dark:bg-gray-700 text-gray-800 dark:text-white transition duration-150"
            />
          </div>
          ...
          <div className="flex justify-end gap-3 mt-6">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-lg font-semibold hover:bg-gray-200 dark:hover:bg-gray-600 transition"
            >
              Cancel
            </button>
            <button
...
export const Teams: React.FC = () => {
  ...
  const renderTeamCard = (team: TeamData, isMember: boolean) => {
    ...
    const canJoin = !isMember && !isLocked;
  ...
  return (
    <BaseLayout className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 dark:text-white mb-2">Teams</h1>
          <p className="text-gray-600 dark:text-gray-300">Connect with friends and stay accountable together</p>
        </div>
        ...
        <div>
          <button
            ...
            className={`inline-flex items-center gap-2 px-6 py-3 rounded-xl font-semibold shadow-sm transition-all duration-200 ${
              isLocked
      ...
      {/* Locked banner */}
      {isLocked && (
        <div className="mb-8 p-4 rounded-xl bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 text-yellow-800 dark:text-yellow-300 shadow-sm">
      ...
      {!isLoading && (
        <>
          {/* My Teams */}
          <div className="mb-8">
            <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-4">My Teams</h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {myTeams.length > 0 ? (
                myTeams.map(team => renderTeamCard(team, true))
              ) : (
                <div className="lg:col-span-3 text-center py-8 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 rounded-xl border border-dashed border-gray-200 dark:border-gray-700">
                  <FaUser className="text-4xl mx-auto mb-3 text-indigo-300 dark:text-indigo-600" />
          ...
          {/* Available Teams */}
          <div>
            <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-4">Discover Teams</h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {discoverTeams.length > 0 ? (
                discoverTeams.map(team => renderTeamCard(team, false))
              ) : (
                <div className="lg:col-span-3 text-center py-8 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 rounded-xl border border-dashed border-gray-200 dark:border-gray-700">
                  <FaUsers className="text-4xl mx-auto mb-3 text-indigo-300 dark:text-indigo-600" />
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-md border border-indigo-100 dark:border-indigo-900/30 hover:shadow-lg transition-all duration-200">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">{members} members</p>
        </div>
      </div>

      <div className="space-y-3 mb-4">
        <div className="flex justify-between items-center">
          <span className="text-sm text-gray-600 dark:text-gray-300">Active Quests</span>
          <span className="font-medium text-gray-800 dark:text-white">{activeQuests}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-sm text-gray-600 dark:text-gray-300">Team XP</span>
          <span className="font-medium text-purple-600 dark:text-purple-400">{teamXP.toLocaleString()}</span>
        </div>
      </div>
      ...
      <button
        onClick={canJoin ? () => onJoin(teamId) : undefined}
        disabled={locked || (canJoin === false && role === 'none')}
        className={`w-full py-2 rounded-lg font-medium transition-all duration-200 ${
          locked || (canJoin === false && role === 'none')
